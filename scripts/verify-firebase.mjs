...
const soft = args.includes("--soft") || process.env.FIREBASE_VERIFY_SOFT === "true";
const baseUrl = (args.find((arg) => !arg.startsWith("--")) || process.env.FIREBASE_APP_URL || "").replace(/\/$/, "");
const retries = Number(process.env.FIREBASE_VERIFY_RETRIES || 6);
const delayMs = Number(process.env.FIREBASE_VERIFY_DELAY_MS || 15000);
const timeoutMs = Number(process.env.FIREBASE_VERIFY_TIMEOUT_MS || 20000);
...
if (!baseUrl) {
  console.error("Missing Firebase App Hosting URL. Usage: npm run verify:firebase -- https://your-app.web.app");
  process.exit(1);
}
...
const checks = [
  ...
  { path: "/api/firebase/status", expected: [200], label: "Firebase status API", json: true },
...
async function runCheck(check) {
  ...
  try {
    ...
    if (check.json && passStatus) {
      ...
      if (check.path === "/api/firebase/status" && data?.platform !== "firebase") {
        jsonPass = false;
        note = "Firebase status did not report platform:firebase";
      }
    }
...
for (let attempt = 1; attempt <= retries; attempt += 1) {
  finalResults = [];
  for (const check of checks) finalResults.push(await runCheck(check));
  const failed = finalResults.filter((item) => !item.pass);
  if (failed.length === 0) break;
  if (attempt < retries) {
    console.log(`Firebase verification attempt ${attempt}/${retries} failed. Retrying in ${delayMs / 1000}s...`);
    await sleep(delayMs);
  }
}

console.log("\nSAR INDUSTRIES NETWORK — Firebase Verification Report");
...
if (failed.length > 0) {
  console.error("Firebase verification found failing routes. Check failed routes above and review Firebase App Hosting rollout logs.");
  if (!soft) process.exit(1);
}

console.log(failed.length === 0 ? "Firebase verification passed." : "Firebase verification completed in soft mode with warnings.");
